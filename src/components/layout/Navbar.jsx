...
export default function Navbar() {
  ...
  const { activeTab, switchTab, role, toggleRole } = useAuthToggle(); 
      ...
      <nav className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white dark:bg-gray-900 shadow-md' : 'bg-white/70 dark:bg-gray-900/70 backdrop-blur'}`}>
        ...
        <div className={`fixed top-0 right-0 w-2/3 h-full bg-gray-800 dark:bg-gray-900 p-4 transition-transform duration-300 ${menuOpen ? 'transform translate-x-0' : 'transform translate-x-full'}`}>
          <div className="flex justify-between items-center">
            <div className="text-2xl font-bold text-white">Holidaze</div>
          ...
          <ul className="flex flex-col mt-8 space-y-4 text-white">
            <li><a href="/" className={`${isActive('/') ? 'font-bold' : ''}`}>Home</a></li>
            <li><a href="/profile" className={`${isActive('/profile') ? 'font-bold' : ''}`}>Profile</a></li>
            ...
            {isLoggedIn ? (
              ...
              <>
                <li><button onClick={() => { openModal('login'); setMenuOpen(false); }} className="text-white">Login</button></li>
                <li><button onClick={() => { openModal('register'); setMenuOpen(false); }} className="text-white">Sign Up</button></li>
              </>
            )}
            <li className="mt-4"><DarkModeToggle /></li>
          </ul>
        </div>
      </nav>
